...
def test_qnh(mission):
    assert mission.weather.altimeter == 760
    mission.weather.altimeter = 754
    assert mission.weather.altimeter == 754
    for wrong_qnh in ['caribou', 719, 791, -1, None, True]:
        with pytest.raises(ValueError):
            mission.weather.altimeter = wrong_qnh
...
def test_wind(mission):
    ...
    assert mission.weather.wind_ground_dir == 0
    assert mission.weather.wind_ground_speed == 0
    for wrong_speed in [-1, 51, True, None, 'caribou']:
        with pytest.raises(ValueError, message=wrong_speed):
            mission.weather.wind_ground_speed = wrong_speed
        with pytest.raises(ValueError, message=wrong_speed):
    ...
    for wrong_dir in [-1, 360, True, None, 'caribou']:
        with pytest.raises(ValueError, message=wrong_dir):
            mission.weather.wind_ground_dir = wrong_dir
        with pytest.raises(ValueError, message=wrong_dir):
...
def test_precipitations(mission):
    ...
    mission.weather.precipitation_code = 0
    for i in range(1, 4):
        with pytest.raises(ValueError, message=i):
            mission.weather.precipitation_code = i
    mission.weather.cloud_density = 5
    mission.weather.precipitation_code = 1
    for i in range(2, 4):
        with pytest.raises(ValueError, message=i):
            mission.weather.precipitation_code = i
    ...
    for i in range(1, 2):
        with pytest.raises(ValueError, message=i):
            mission.weather.precipitation_code = i
    mission.weather.precipitation_code = 4
    # Season code has been deprecated by ED
    # miz.mission.weather.season_code = 1
    # assert miz.mission.weather.temperature == 5
    # assert miz.mission.weather.precipitation_code == 2
    # miz.mission.weather.season_code = 2
    # miz.mission.weather.temperature = -20
    # assert miz.mission.weather.precipitation_code == 4
    # miz.mission.weather.precipitation_code = 3
    # miz.mission.weather.season_code = 1
    # assert miz.mission.weather.precipitation_code == 1
